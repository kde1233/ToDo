...
# 이 피일은 '할 일(Task)' 데이터를 어떻게 주고받을지를 정의한 곳이다.
...
# - 할 일 관련 데이터 구조들이 공통으로 사용하는 부분을 따로 몪은 클래스
# - TaskCreate, TaskCreateResponse, Task가 이 클래스르 상속해서 사용한다
# --------------------------------------
class TaskBase(BaseModel):
    title: str | None = Field(
        default=None, # 값이 없을 수도 있으므로 기본값을 None으로 설정함
        examples=["세탁소에 맡긴 것을 찾으로 가기"] # 예시 데이터 (API 문서에 보여짐)
    )
    # * title: 할 일의 제목
    ...
    # - title만 필요하므로 TaskBase를 그대를 상속해서 사용함
    # ------------------------
...
class TaskCreateResponse(TaskCreate):
    ...
    # * SQLAlchemy 같은 ORM 객체를 이 모델을 자동 변환할 수 있게 설정
