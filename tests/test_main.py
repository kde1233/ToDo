...
# HTTP 상태 코드를 직관적인 이름으로 시용하기 위해 불러옴
...
async def async_client() -> AsyncGenerator[AsyncClient, None]:
    ...
    async_session = sessionmaker(
        autocommit=False,   # 명시적으로 commit 해야 저정됨
    ...
    # -----------------------------
    # 3. get_db() 함수를 테스트용 DB와 연결하도록 override
...
async def test_create_and_read(async_client):
    ...
    assert response_obj[0]["title"] == "테스트 작업" # 그 할 일의 제목의 정확한지 확인 
...
async def test_done_flag(async_client):
    ...
    # - title은 정상적으로 입력하고,
    # due_date에는 존재하지 않은 날짜(12월 32일)를 넣어 테스트합니다.
    # -------------
